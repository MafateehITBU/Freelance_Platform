...
export const addFreelancer = async (req, res) => {
    try {
        ...
        if (files?.personalIdImage?.[0]) {
            ...
            fs.unlinkSync(files.personalIdImage[0].path);
        } else {
        ...
        const newFreelancer = new Freelancer({
            ...
            phone,
            dateOfBirth,
            profilePicture: profilePictureUrl, // optional
            personalIdImage: personalIdImageUrl, // required
            portfolio: portfolioUrls // optional array of URLs
        });
...
export const toggleFreelancerVerification = async (req, res) => {
    try {
        ...
        const mailOptions = {
            from: `"Freelance Platform" <${process.env.EMAIL_FREELANCER}>`,
            to: freelancer.email,
            subject,
            text,
        };
